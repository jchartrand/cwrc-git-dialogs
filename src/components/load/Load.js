			...
			<Modal.Body>
				...
				>
					<Tab eventKey="templates" title="CWRC Templates" style={{marginTop: '10px'}}>
						{loading ? (
							<h5><Glyphicon glyph="cloud-download" style={{ padding: '10px' }}/> Loading...</h5>
						) : error !== '' ? (
							<div><h5>Error!</h5><p>{error}</p></div>
						) : (
							<ListGroup>
								{templates.map((item, key) => (
									<ListGroupItem
										key={key}
										onClick={() => onFileUpload(item.download_url)}
									>
									...
									>
										<Panel eventKey="private">
											...
											<Panel.Collapse>
												<Panel.Body>
													<ControlLabel>
														Show repositories for which I am:
													</ControlLabel>
													<ToggleButtonGroup
														type="radio"
														name="affiliation"
